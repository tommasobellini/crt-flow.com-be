...
from indicators import calculate_atr, get_historical_seasonality, calculate_poc
...
def detect_macro_sweep(ticker, df, tf, config=None):
    ...
    try:
        ...
        # Analizziamo l'ULTIMA CANDELA CHIUSA (iloc[-2]), non quella live in corso!
        curr = df.iloc[-2]
...
def detect_smc_orderpairing(ticker, df, tf, config=None, htf_pools=None):
    ...
    try:
        # 1. Calcolo ATR per il buffer dello Stop Loss (Usiamo la candela chiusa)
        atr_series = ta.atr(df['High'], df['Low'], df['Close'], length=14)
        if atr_series is None or atr_series.empty: return None
        atr_val = float(atr_series.iloc[-2])

        # Analizziamo l'ULTIMA CANDELA CHIUSA (iloc[-2])
        ...
        # --- THE WICK SHIELD (Anti-Inducement Filter) ---
        # Controlliamo le ultime 15 candele prima della nostra (circa mezza giornata/una giornata intera di contrattazioni)
        lookback_window = df.iloc[-17:-2]
        
        # Salviamo il massimo e il minimo assoluto di questa finestra
        recent_max = to_f(lookback_window['High'].max())
        recent_min = to_f(lookback_window['Low'].min())

        # --- SETUP SHORT (Bearish TurtleSoup / OHP) ---
        # 1. Deve essere Rossa (c_close < c_open)
        # 2. DISPLACEMENT: Chiude nella metà inferiore
        if c_close < c_open and c_close <= (c_low + c_range * 0.5):
            
            # WICK SHIELD CHECK: Se c'è una wick più alta nelle ore precedenti, NON entrare! 
            # I market maker andranno a prenderla.
            ...
            for p_name, p_val in htf_pools.items():
                ...
                if "H" in p_name: 
                    if c_high > p_val and max(c_open, c_close) < p_val:
                        swept_level = p_name
                        break
            ...
            if swept_level:
                ...
                # Stop Loss: Massimo assoluto + 1.0 ATR (Stanza extra per respirare)
                sl = c_high + (atr_val * 1.0)
                ...
                # --- NUOVO: CONTROLLO VOLUME PROFILE (POC) ---
                # Calcoliamo il Point of Control delle ultime 50 candele (es. ultima settimana su 1H)
                poc_price = calculate_poc(df, lookback=50, bins=50)
                
                confluence_text = f"Orderpairing 50% on {swept_level} Purged"
                diamond_score = "A++"
                
                if poc_price is not None:
                    # Calcoliamo la distanza percentuale tra la nostra entrata e il POC
                    dist_to_poc = abs(entry - poc_price) / entry
                    
                    # Se l'entrata è entro lo 0.5% dal livello di massimo volume istituzionale...
                    if dist_to_poc <= 0.005: 
                        confluence_text += " + POC Alignment 🛡️"
                        diamond_score = "A+++ (God Mode)" # Alziamo il grado a livello massimo
                # ---------------------------------------------

                # --- STEP 1: FIBONACCI OTE (DEEP DISCOUNT/PREMIUM) ---
                macro_window = df.iloc[-100:-2]
                macro_high = to_f(macro_window['High'].max())
                macro_low = to_f(macro_window['Low'].min())
                macro_range = macro_high - macro_low
                
                if macro_range > 0:
                    fib_ote_bearish = macro_low + (macro_range * 0.705)
                    if entry >= fib_ote_bearish:
                        confluence_text += " + OTE Premium 🎯"
                # ---------------------------------------------------

                return create_smc_signal(ticker, tf, "bearish_tbs", confluence_text, c_high, c_low, entry, sl, tp, d_score=diamond_score, confluence=confluence_text)

        # --- SETUP LONG (Bullish TurtleSoup / OLP) ---
        # 1. Deve essere Verde (c_close > c_open)
        # 2. DISPLACEMENT: Chiude nella metà superiore
        elif c_close > c_open and c_close >= (c_high - c_range * 0.5):
            
            # WICK SHIELD CHECK (Quello del tuo screenshot!): 
            # Se c'è una wick gigante più bassa sotto di noi, il setup è una trappola!
            if recent_min < c_low:
                return None
            ...
            for p_name, p_val in htf_pools.items():
                if p_val is None: continue
                if "L" in p_name: 
                    if c_low < p_val and min(c_open, c_close) > p_val:
                        swept_level = p_name
                        break
            ...
            if swept_level:
                ...
                # Stop Loss: Minimo assoluto - 1.0 ATR (Protezione Stop Hunt)
                sl = c_low - (atr_val * 1.0)
                ...
                if macro_range > 0:
                    fib_ote_bullish = macro_high - (macro_range * 0.705)
                    if entry <= fib_ote_bullish:
                        confluence_text += " + OTE Discount 🎯"
                # ---------------------------------------------------

                return create_smc_signal(ticker, tf, "bullish_tbs", confluence_text, c_high, c_low, entry, sl, tp, d_score=diamond_score, confluence=confluence_text)
...
def create_smc_signal(ticker, tf, s_type, subtype, high, low, entry, sl, tp, d_score="A++", confluence="Orderpairing 50%"):
    ...
    return {
        ...
        "diamond_score": d_score, "confluence_level": confluence,
...
def detect_golden_wick(ticker, df, tf, config=None):
    ...
    try:
        ...
        if lower_wick > (c_range * 0.45) and c_close > (c_low + c_range * 0.60):
            ...
            signal_data['trigger_candles'] = json.dumps([int(df.index[-2].timestamp())])
    ...
    for sig in signals:
        ...
        if status == 'pending':
            ...
            if triggered:
                ...
                # NOVITÀ: Usciamo subito! Non valutiamo il TP/SL sulla stessa candela 
                # che ci ha innescato, per evitare falsi positivi di volatilità intraday.
                continue 
            elif missed:
        ...
        if should_expire:
            # DEBUG LOG PER CAPIRE COSA SUCCEDE (Autopsia del trade)
            logger.info(f"🔍 DEBUG TRIGGER: {ticker} | Entry: {entry} | SL: {new_sl} | TP: {tp}")
            logger.info(f"🔍 DEBUG CANDLE: High: {curr_high} | Low: {curr_low} | Close: {curr_close}")
