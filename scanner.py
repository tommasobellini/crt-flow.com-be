...
def main():
        ...
        for tf, cfg in TF_CONFIG.items():
            try:
                ...
                for ticker in chunk:
                    ...
                    if sig: all_detected_signals.append(sig)
                    ...
                    if tf == "1H" and not df.empty:
                        ...
                        if sig_4h: all_detected_signals.append(sig_4h)
    ...
    # 4. SALVATAGGIO SU SUPABASE (BULK)
    if all_detected_signals:
        try:
            logger.info(f"Trovati {len(all_detected_signals)} segnali. Aggiornamento database...")
            
            # Reset segnali vecchi (EOD o sessione precedente)
            supabase.table("crt_signals").update({"is_active": False}).eq("is_active", True).execute()
            ...
            for j in range(0, len(all_detected_signals), 1000):
                ...
                try:
                    ...
                    if "price" in str(e) or "column" in str(e):
                        ...
                        # Rimuovi 'price' dai dizionari e riprova
                        batch_no_price = [{k: v for k, v in s.items() if k != 'price'} for s in batch]
                        supabase.table("crt_signals").insert(batch_no_price).execute()
                        logger.info("Upload completato (senza prezzo). NOTA: Aggiungi colonna 'price' al DB per i tooltip.")
                    else:
                        raise e
        ...
        logger.info("Nessun segnale rilevato in questa scansione.")
